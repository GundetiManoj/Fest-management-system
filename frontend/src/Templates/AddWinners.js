...
export default function AddWinners(props) {
    // need to select 3 winners for each event
    // if event has winners set then show them
    // else show a form to add winners
    ...
    // we jus need ids of winners
    const HandleSubmit = () => {
        axios.post('http://localhost:5002/setwinners', { eventId: eventId, winner1: winner1, winner2: winner2, winner3: winner3 },{
            headers : {
    ...
    return (
        <div className='AddWinners'>
            <h1>Add Winners</h1>
            <div className='AddWinners-List'>
                <select onChange={(e) => {setWinner1(e.target.value)}}>
                    <option value=''>Select Winner 1</option>
                    {eventData.map((participant) => {
                        return (
                            <option value={participant.id}>{participant.emailid}</option>
                        )
                    })}
                </select>
                <select onChange={(e) => {setWinner2(e.target.value)}}>
                    <option value='' >Select Winner 2</option>
                    {eventData.map((participant) => {
                        return (
                            <option value={participant.id}>{participant.emailid}</option>
                        )
                ...
                <select onChange={(e) => {setWinner3(e.target.value)}}>
                    <option value='' >Select Winner 3</option>
                    {eventData.map((participant) => {
                        return (
                            <option value={participant.id}>{participant.emailid}</option>
                        )
